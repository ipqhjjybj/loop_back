...
#ifndef _LivermoreStrategyTest2
#define _LivermoreStrategyTest2
...
class LivermoreStrategyTest2 : public ctaTemplate{
...
public:
    ...
    int maLengthLong;       // 时序均线来进行过滤 多单
    int maLengthShort;      // 时序均线来进行过滤 空单
    ...
    vector<double> filterMaLong, filterMaShort;
    int refreshFlag;
    int ssqs_jishu;
    int xjqs_jishu;
    ...
    LivermoreStrategyTest2(){
        ctaTemplate();
        this->setStrategyName("LivermoreStrategyTest2");
    }
    ...
    vector<string> getCanshuSetNames(){
        ...
        vCanshu.push_back("maLengthLong");
        vCanshu.push_back("maLengthShort");
    ...
    void init(){
        ...
        filterMaLong.reserve(n + 10);
        filterMaShort.reserve( n + 10);
    ...
    void setCanshuFromVec(const vector<double> &arr){
        this->param1 = arr[0]; this->param2 = arr[1];  this->z1 = arr[2] ; this->z2 = arr[3] ; this->maLengthLong = arr[4] ; this->maLengthShort = arr[5];
    }
    ...
    TradingResultNode getTradingResult( const char *_code , const char *_period,const char *start_date , const char *end_date){
        ...
        d.push_back(this->z1);
        d.push_back(this->z2);
        d.push_back(this->maLengthLong);
        d.push_back(this->maLengthShort);
        a.setCanshuSet(d);
        return a;
    }
    ...
    void onBar(int i){
        ...
        if( this->pos == 0){
            ...
            if(buy_cond == 1 && 0 == this->refreshFlag ){
                if( i > 1 && closeArray[i] > filterMaLong[i] && closeArray[i] > filterMaLong[i-1]){
                    this->Buy(this->lots , closeArray[i] , i);
                }
            }
            if(sell_cond == 1 && 0 == this->refreshFlag ){
                if( i > 1 && closeArray[i] < filterMaShort[i] && closeArray[i] < filterMaShort[i-1]){
            ...
            if( BarsSinceEntry > 0 && lowArray[i] < this->high_since_entry * ( 1 - this->z1)){
                this->refreshFlag = 1;
                this->Sell(abs(this->pos) , closeArray[i] , i);
            }
            ...
            if( BarsSinceEntry > 0 && highArray[i] > this->low_since_entry * ( 1 + this->z2)){
                this->refreshFlag = 1;
                this->BuyToCover(abs(this->pos) , closeArray[i] , i);
            }
        }
    ...
    //filterMaLong, filterMaShort
    void beforeRun( ){
        ctaTemplate::beforeRun();

        Average(filterMaLong, closeArray , maLengthLong , n);
        Average(filterMaShort , closeArray , maLengthShort , n);
    }
    ...
    void loopForce(){
        ...
        if( (fp = fopen(loopForceFileName.c_str(), "w")) != NULL){
            for(double t_param1 = 0.008; t_param1 < 0.040;t_param1 += 0.002){
                for(double t_param2 = 0.004;t_param2 < 0.040; t_param2 += 0.002){
                    for(double t_z1 = 0.003; t_z1 < 0.100; t_z1 += 0.002){
                        for(double t_z2 = 0.003; t_z2 < 0.100 ; t_z2 += 0.001){
                            for(int maLengthLong = 2 ; maLengthLong < 30; maLengthLong += 2){
                                for(int maLengthShort = 2; maLengthShort < 30; maLengthShort += 2){
                                    ...
                                    tmp.push_back( maLengthLong * 1.0);
                                    tmp.push_back( maLengthShort * 1.0);
