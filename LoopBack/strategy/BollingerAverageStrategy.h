...
#ifndef _BollingerAverageStrategy
#define _BollingerAverageStrategy
 ...
 *  策略，劲道指数
 */
class BollingerAverageStrategy : public ctaTemplate{
...
public:
    ...
    /////////////////////////////////////////////////////////////
    //申明参数
    int boll_length;        //布林通道
    double bollDev;         //布林Dev

    ///////////////////////////////////////////////////////////////

    vector<double> bollAve;
    vector<double> bollStd;
    double buyUp;
    double sellUp;
    ...
    BollingerAverageStrategy(){
        this->setStrategyName("BollingerAverageStrategy");
        ctaTemplate();
    }
    ...
    TradingResultNode getTradingResult( const char *_code , const char *_period,const char *start_date , const char *end_date){
        ...
        d.push_back( boll_length); d.push_back( bollDev);
    ...
    void onBar(int i)
    {
        ...
        buyUp = bollAve[i] + bollStd[i] * bollDev ;
        sellUp = bollAve[i] - bollStd[i] * bollDev ;

        if( closeArray[i] >= buyUp ){
            signalLong = 1;
        }

        if( closeArray[i] <= sellUp){
            signalLong = -1;
        }
    ...
    void beforeRun( )
    {
        ...
        bollAve.clear(); bollStd.clear();  
        bollAve.reserve(n + 10);  bollStd.reserve(n + 10);

        // Strategy Variable
        xAverage( bollAve , closeArray , boll_length , n);
        AvgTrueRange( bollStd , highArray, lowArray , closeArray , boll_length , n);
    }
    ...
    virtual void loopForce(){
        ...
        if( (fp = fopen(loopForceFileName.c_str(), "w")) != NULL){
            for( int boll_length = 4; boll_length <= 50 ;boll_length++){
                for( double bollDev = 1.0; bollDev <= 4.0; bollDev += 0.1){
                    vector<double> tmp; tmp.clear();
                    tmp.push_back(boll_length);
                    tmp.push_back(bollDev);
...
#endif // _BollingerAverageStrategy
