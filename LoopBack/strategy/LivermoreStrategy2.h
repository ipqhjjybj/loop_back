...
#ifndef _LivermoreStrategy2
#define _LivermoreStrategy2
...
#endif
class LivermoreStrategy2 : public ctaTemplate{
...
public:
    ...
    vector<const string * > distinct_big_condArray;                //不同的大方向状态
    ...
    LivermoreStrategy2(){
        ctaTemplate();
        this->setStrategyName("livermoreStrategy2");
    }
    ...
    void init(){
        ...
        big_condArray.clear();
        big_condArray.reserve(n + 10);
        distinct_big_condArray.clear();
        distinct_big_condArray.reserve(n + 10);
    ...
    TradingResultNode getTradingResult( const char *_code , const char *_period,const char *start_date , const char *end_date){
        //TradingResultNode a = TradingResultNode( _code, _period ,start_date, end_date ,tradingResult,tradingNum , account_money ,slippage * n_slippages, rate , perSize , cash_deposit, vecTradeDay , vecTradeWeekDay , dayIncome , fee_type);
    ...
    void onBar(int i){
        ...
        if(this->distinct_big_condArray.size() == 0 || (this->distinct_big_condArray.size() > 0 and this->distinct_big_condArray[this->distinct_big_condArray.size()-1] != this->big_condition )){
            this->distinct_big_condArray.push_back(this->big_condition);
        }
        ...
        if( this->distinct_big_condArray.size() > 2 && this->big_condition == &ZiRanHuiShen
            && this->distinct_big_condArray[this->distinct_big_condArray.size() - 2] == &ZiRanHuiChe
            && this->distinct_big_condArray[this->distinct_big_condArray.size() - 3] == &ShangShenQuShi){
            buy_cond  = 1;
        }
        if( this->pos > 0 && this->big_condition == &XiaJiangQushi){
            sell_cond = 1;
        }
        if( this->distinct_big_condArray.size() > 2 && this->big_condition == &ZiRanHuiChe
            && this->distinct_big_condArray[this->distinct_big_condArray.size() - 2] == &ZiRanHuiShen
            && this->distinct_big_condArray[this->distinct_big_condArray.size() - 3] == &XiaJiangQushi ){
            short_cond = 1;
        }
        if( this->pos < 0 && this->big_condition == &ShangShenQuShi){
            cover_cond = 1;
        }

        if(this->distinct_big_condArray.size() > 2 && this->pos > 0 && this->big_condition == &ZiRanHuiChe && this->distinct_big_condArray[this->distinct_big_condArray.size() - 2] == &ShangShenQuShi){
            sell_cond = 1;
        }
        if(this->distinct_big_condArray.size() > 2 && this->pos < 0 && this->big_condition == &ZiRanHuiShen && this->distinct_big_condArray[this->distinct_big_condArray.size() - 2] == &XiaJiangQushi){
            cover_cond = 1;
        }
        if( this->pos == 0){
            if(buy_cond == 1){
    ...
    void loopForce(){
        ...
        if( (fp = fopen(loopForceFileName.c_str(), "w")) != NULL){
            for(double t_param1 = 0.002; t_param1 < 0.040;t_param1 += 0.002){
                for(double t_param2 = 0.002;t_param2 < 0.040; t_param2 += 0.002){
