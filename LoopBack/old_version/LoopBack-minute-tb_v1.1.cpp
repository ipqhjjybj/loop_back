...
int pos;
int mp[MAXN + 10];                  // 总持仓
double free_money[MAXN + 10];       // 空闲资金
double total_money[MAXN + 10];      // 总资金 (空闲资金 + 持仓)
int BarsSinceEntry;                 // 账户进入

/////////////////////////////////////////////////////////////
int CF = 5 , CM = 8 , CS = 13 , d_CF = 3 , d_CM = 5 , d_CS = 8;
int N_up = 10 , N_down = 10;
double zhisunlv_l = 10 , zhisunlv_s = 10 ;
///////////////////////////////////////////////////////////////

double slippage = 1.0 , rate = 1.29/10000 , perSize = 10 , minMovement = 1.0;
    ...
    for( int i = 1; i < n;i++){ // i = BufferSize - 1
        ...
        if(eyuArray[i] < 0 && lowArray[i] < kai_down && mp[i] > -1){
            if(openArray[i] < kai_down){
                ...
                #ifdef debug
                printf("date %s time %s ,SellShort price %lf , volume %lf\n", g_date[i] , g_time[i] ,kai_down , 1.0*lots);
                #endif // debug
        ...
        if( mp[i] > 0 && lowArray[i] < zhisun_l && BarsSinceEntry > 0){
            if( openArray[i] < zhisun_l){
                ...
                #ifdef debug
                printf("date %s time %s ,Sell price %lf , volume %lf\n", g_date[i] , g_time[i] ,zhisun_l , 1.0*lots);
                #endif // debug
        ...
        if( mp[i] < 0 && highArray[i] > zhisun_s && BarsSinceEntry > 0){
            if( openArray[i] > zhisun_s){
                ...
                #ifdef debug
                printf("date %s time %s ,BuyToCover price %lf , volume %lf\n", g_date[i] , g_time[i] ,zhisun_s , 1.0*lots);
                #endif // debug
